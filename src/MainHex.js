...
export function MainHex({count, mouse, colors, props}) {
  const top = useRef(0);
  ...
  useFrame((state) => {
    ...
    particles.forEach((particle, i) => {
      ...
      //console.log(nodes[key].translateTotal);
      ...
      //console.log(transY);
      if (((nodes[key].translateTotal+transY)>0)&&((nodes[key].translateTotal+transY)<0.95)){
        nodes[key].translateTotal+=transY
        nodes[key].translateY(transY)
      }
      else if ((nodes[key].translateTotal+transY)>1.25){
  ...
  const material1 = new THREE.MeshPhongMaterial({  color: 0x4bf1e3, emissive:0x4bf1e3, transparent: 1,emissiveIntensity:.5, name: "Emit" })//new THREE.MeshStandardMaterial({ color: 0xdddddd, opacity:0})
  ...
  return (
    ...
    <instancedMesh scale = {[.578,.575,.578]} ref = {group}  args={[nodes.Hex13.geometry, materials.Silver, count]}>
        <group  >
          <instancedMesh ref={top}

          args={[nodes.Cylinder021.geometry, materials.Silver, count]}>



          </instancedMesh>
          <instancedMesh ref={bottom}  args={[nodes.Cylinder021_1.geometry, material1, count]}>
